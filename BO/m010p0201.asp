...
<head>
	...
	function Init() {
		...
		selectChange(frm10p02.Operand, frm10p02.Operator,detailData);
	<%
	if (String(Request.QueryString("Search")) == "true") { 		
	%>
		document.frm10p02.SearchResult.focus();
	<%
	} else {
	%>
		document.frm10p02.Operand.focus();
	<%
	}
	%>		
	}
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		document.frm10p02.StringSearchTextBoxOne.value="";
		document.frm10p02.StringSearchTextBoxTwo.value="";
		var myEle ;
		for (var q=controlToPopulate.options.length;q>=0;q--) controlToPopulate.options[q]=null;
		myEle = document.createElement("option") ;
		var y = control.value;
		if (y == "141"){
			document.frm10p02.StringSearchTextBoxTwo.disabled = false;
			oStg2.style.visibility = "visible";
		} else {
			document.frm10p02.StringSearchTextBoxTwo.disabled = true;
			oStg2.style.visibility = "hidden";
		}
		if (y != 0) {
	...
	function Togo() {
		var j = document.frm10p02.Operand[document.frm10p02.Operand.selectedIndex].value ;
		var l = document.frm10p02.Operator[document.frm10p02.Operator.selectedIndex].value	
		document.frm10p02.MM_curOprd.value = j ;
		document.frm10p02.MM_curOptr.value = l ;
		document.frm10p02.MM_flag.value = true ;
	}

	function CnstrFltr() {
		var stgPgQuery = "";
		
		var chvOprd = document.frm10p02.Operand[document.frm10p02.Operand.selectedIndex].value ; 
		var chrNot  = "";
		var chvOptr = document.frm10p02.Operator[document.frm10p02.Operator.selectedIndex].value ;
		var chvStg1 = document.frm10p02.StringSearchTextBoxOne.value;
		var chvStg2 = document.frm10p02.StringSearchTextBoxTwo.value;

		if ((chvOprd=="141") && (chvOptr!="0")) {
			if (chvStg1 == "") {
				alert("Enter Start Date");
				document.frm10p02.StringSearchTextBoxOne.focus();
				return ;
			}
			if (chvStg2 == "") {
				alert("Enter End Date");
				document.frm10p02.StringSearchTextBoxTwo.focus();
				return ;
			}
		...
		if (chvOptr == "0") {
			...
			stgPgQuery += "m010p0201.asp?Search=true&inspSrtBy=1&inspSrtOrd=0&chvFilter=" + stgFilter ;
		}
		document.frm10p02.action = stgPgQuery;
		document.frm10p02.submit();
	}
	
	function SelectInstitution(){
		if (document.frm10p02.SearchResult.selectedIndex==-1){
			alert("Select an institution.")
			return ;
		}	

		opener.document.frm0101.BuyerType.value="4";	
		opener.document.frm0101.InstitutionBuyerID.value=document.frm10p02.SearchResult[document.frm10p02.SearchResult.selectedIndex].value;
		opener.document.frm0101.InstitutionBuyerName.value=document.frm10p02.SearchResult.options[document.frm10p02.SearchResult.selectedIndex].text;
		self.close();
	}
	</script>
</head>
<body onload="Init();">
<form name="frm10p02" method="post" action="">
<h5>Search Criteria</h5>
<table cellpadding="1" cellspacing="1">
	<tr> 
		<td nowrap><select name="Operand" onchange="selectChange(this, frm10p02.Operator,detailData);" tabindex="1" accesskey="F">
				<% 
				while (!rsOprd.EOF) {
	...
	<tr>
		<td nowrap colspan="4"><input type="button" value="Proceed" onClick="CnstrFltr();" tabindex="6" class="btnstyle"></td>		
	</tr>
</table>
<hr>
<table cellpadding="1" cellspacing="1">
