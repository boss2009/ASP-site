...
if (String(Request("MM_insert")) != "undefined") {
	...
	rsDesktop.Source = "{call dbo.cp_Insert_DeskTop(" + Session("insStaff_id") + "," + Request.Form("intBuyout_Req_id") + ",25,'" + CurrentDate() + "',0)}";
...
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td><select name="SaveTo" tabindex="1" accesskey="F">
			...
			while (!rsModule.EOF) {
			%>
				<option value="<%=(rsModule.Fields.Item("insId").Value)%>" <%=((rsModule.Fields.Item("insId").Value == 25)?"SELECTED":"")%> ><%=(rsModule.Fields.Item("ncvMODname").Value)%>
			<%
				rsModule.MoveNext();
			}
			%>
