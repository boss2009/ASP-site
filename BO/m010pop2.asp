...
if (String(Request.Form("MM_update"))=="true"){
	...
	if (Request.Form("BoxCount") > 1) {
		for (i = 1; i <= Request.Form("BoxCount"); i++){
			if (Request.Form("BoxWeight")(i) > 0) {
				rsUpdateBoxes.Source = "{call dbo.cp_buyout_ship_box("+Request.Form("ID")(i)+","+Request.QueryString("intShip_dtl_id")+",0,"+Request.Form("BoxWeight")(i)+",0,0,0,'E',0)}";	
				rsUpdateBoxes.Open();			
			} else {
				rsUpdateBoxes.Source = "{call dbo.cp_buyout_ship_box("+Request.Form("ID")(i)+","+Request.QueryString("intShip_dtl_id")+",0,0,0,0,0,'D',0)}";	
				rsUpdateBoxes.Open();						
			}
		}
	}
	...
	if (Request.Form("BoxCount") == 1) {
		if (Request.Form("BoxWeight") > 0) {	
			rsUpdateBoxes.Source = "{call dbo.cp_buyout_ship_box("+Request.Form("ID")+","+Request.QueryString("intShip_dtl_id")+",0,"+Request.Form("BoxWeight")+",0,0,0,'E',0)}";	
			rsUpdateBoxes.Open();			
		} else {
			rsUpdateBoxes.Source = "{call dbo.cp_buyout_ship_box("+Request.Form("ID")+","+Request.QueryString("intShip_dtl_id")+",0,0,0,0,0,'D',0)}";	
			rsUpdateBoxes.Open();						
		}
	}
		
	if (Request.Form("NewBoxWeight") > 0) {
		rsUpdateBoxes.Source = "{call dbo.cp_buyout_ship_box(0,"+Request.QueryString("intShip_dtl_id")+",0,"+Request.Form("NewBoxWeight")+","+Request.QueryString("intBuyout_req_id")+",0,0,'A',0)}";
		rsUpdateBoxes.Open();				
	}	
}
...
rsBoxes.Source = "{call dbo.cp_buyout_ship_box(0,"+Request.QueryString("intShip_dtl_id")+",0,0,0,0,0,'Q',0)}";
...
<head>
	...
	function Close(){
		...
		if (!opener.closed) {		
			opener.frm0501.NumberOfBoxes.value=document.frmpop2.BoxCount.value;
			opener.frm0501.TotalWeight.value=TotalWeight;
		}
		window.close();
	}
	</script>
