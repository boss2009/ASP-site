...
rsNotes.Source = "{call dbo.cp_buyout_request_notes("+ Request.QueryString("intBuyout_req_id") + ",0,'',0,'',0,'Q',0)}";
...
<html>
<head>
	<title>Equipment Requested Notes</title>
...
<body>
<h5>Equipment Requested Notes</h5>
<hr>
<table cellspacing="1" cellpadding="2" class="Mtable">
	<tr> 	
		<th class="headrow" align="left" nowrap>Note Type</th>		
...
%>
    <tr> 
		<td valign="top"><a href="m010e0402.asp?int_Note_id=<%=(rsNotes.Fields.Item("int_Note_id").Value)%>&intBuyout_req_id=<%=Request.QueryString("intBuyout_req_id")%>"><%=(rsNotes.Fields.Item("chvType_of_Note").Value)%></a></td>
...
switch (ChkNotes.Parameters.Item("@insRtnFlag").Value) {
	case 0:
%>
<a href="javascript: openWindow('m010a0402.asp?intBuyout_req_id=<%=Request.QueryString("intBuyout_req_id")%>','wA0402');">Add Requested Notes</a>
<%
	break ;
	case 2:
%>
