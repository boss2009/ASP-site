...
rsNotes.Source = "{call dbo.cp_buyout_sold_notes("+ Request.QueryString("intBuyout_req_id") + ",0,'',0,'',0,'Q',0)}";
...
ChkNotes.CommandText = "dbo.cp_Chk_Buyout_Note";
...
ChkNotes.Parameters.Append(ChkNotes.CreateParameter("@insPR_id", 3, 1,10000,Request.QueryString("intBuyout_req_id")));
ChkNotes.Parameters.Append(ChkNotes.CreateParameter("@insRtnFlag", 2, 2));
ChkNotes.Execute();
%>
<html>
<head>
	<title>Equipment Sold Notes</title>
...
<% 
while ((!rsNotes.EOF) && ((ChkNotes.Parameters.Item("@insRtnFlag").Value==2) || (ChkNotes.Parameters.Item("@insRtnFlag").Value==3))) { 
%>
    <tr> 
		<td valign="top"><a href="m010e0403.asp?int_Note_id=<%=(rsNotes.Fields.Item("int_Note_id").Value)%>&intBuyout_req_id=<%=Request.QueryString("intBuyout_req_id")%>"><%=(rsNotes.Fields.Item("chvType_of_Note").Value)%></a></td>
...
switch (ChkNotes.Parameters.Item("@insRtnFlag").Value) {
	case 0:
%>
<a href="javascript: openWindow('m010a0403.asp?intBuyout_req_id=<%=Request.QueryString("intBuyout_req_id")%>','wA0403');">Add Sold Notes</a>
<%
	break ;
	case 1:
%>
