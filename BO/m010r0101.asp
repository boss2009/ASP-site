...
<head>
	...
	%>
			Grp4Data[239][<%=rsCaseManager("insId")%>] = "<%= rsCaseManager("chvName") %>"
	<%
	...
	%>
			Grp4Data[247][<%=rsShippingMethod("intship_method_id")%>] = "<%= rsShippingMethod("chvname") %>"
	<%
	...
	%>
		  Grp4Data[236][<%=rsBuyoutStatus("insbuyout_status_id")%>] = "<%= rsBuyoutStatus("chvBuyout_status") %>"
	<%
	...
	%>
			Grp4Data[237][<%=rsFundingSource("insFunding_source_id")%>] = "<%= rsFundingSource("chvfunding_source_name") %>"
	<%
	...
	%>
			Grp4Data[235][<%=rsInstitution("insSchool_id")%>] = "<%= rsInstitution("chvSchool_Name") %>"
	<%
	...
	function CnstrFltr(output){
		...
		if (output==1) {
			document.frm10s01.action = "m010q04.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;		
			document.frm10s01.submit() ; 		
		} else {
			var ExcelSearch = window.open("m010q04excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
...
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td nowrap><DIV ID="oStg12" STYLE="visibility:hidden">
			...
			<input type="text" name="StringSearchTextboxTwo" size="15" tabindex="5">
			<span style="font-size: 7pt">(mm/dd/yyyy)</span>
		</DIV></td>
    </tr>
</table>
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td nowrap><DIV ID="oOptrd21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperand" onchange="selectChange(this, frm10s01.LookupValueSearchOperator,detailData);" tabindex="7">
			<% 
			while (!rsOprd2.EOF) { 
			%>
		...
		<td nowrap><DIV ID="oOptr21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperator" onchange="selectChange4(this, frm10s01.LookupValueSearchOptions,Grp4Data );" tabindex="8"></select>
		</DIV></td>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		...
		<td nowrap valign="top"><DIV ID="oOptrd41" STYLE="visibility:visible"> 
			<select name="MultiSelectOperand" OnChange="initializeList(document.frm10s01.MultiSelectOperand,document.frm10s01.MultiSelectOptions)" tabindex="15">
			<% 
			while (!rsOprd4.EOF) {
			%>
