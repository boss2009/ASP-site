...
<table width="725" border="0" class="HdrReg">
  ...
  <tr> 
    <td colspan="3"> 
      <div align="left"><%=(rsClient.Fields.Item("chvName").Value)%> is in default of the established equipment loan plan dated 
        [--date--] from the Assistive Technology - British Columbia. We have made numerous requests 
        and attempts to retrieve the equipment. I am requesting that this letter 
        be placed in both the VRS file and the [POST-SECONDARY] file for <%=(rsClient.Fields.Item("chvName").Value)%>, as a reminder that <%=(rsClient.Fields.Item("chvFst_Name").Value)%> is not in good standing with Assistive Technology - British Columbia.</div>
    </td>
    <td width="49">&nbsp;</td>
  </tr>
  <tr> 
  ...
  <tr> 
    <td colspan="3">This equipment in on loan for educational purposes only and 
      ...
      therefore,<%=(rsClient.Fields.Item("chvFst_Name").Value)%> no longer meets eligibility criteria. Assistive Technology - British Columbia 
...
%>
  ...
  <tr> 
    <td colspan="3">We require either arrangements to be made for the return of 
      equipment, or payment in the amount of dollars ($0.00) to rescind the provincial 
  ...
  <tr> 
    <td width="42">&nbsp;</td>
    <td colspan="3">Sandy Rodgers, Senior Coordinator, Ministry Human Resources</td>
  </tr>
  <tr> 
    <td width="42">&nbsp;</td>
    <td colspan="3">Shirley Marshall, A.Coordinator, Special Programs, AVED, Student 
      Services Branch</td>
  </tr>
  <tr> 
    <td width="42">&nbsp;</td>
