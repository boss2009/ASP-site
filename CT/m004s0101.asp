...
<html>
<head>
	<title>Contact - Quick Search</title>
	...
	rsContactType.Open();
	// Load the Contact Type Lookup 
	if (!rsContactType.EOF){ 
	   Response.Write("Grp4Data[106] = new Array();")	
	   while (!rsContactType.EOF) { 
		  chrData = rsContactType("chvWork_type_desc")
		  intID   = rsContactType("intWork_type_id")
	%>
		  Grp4Data[106][<%=intID%>] = "<%= chrData %>";
	<%
	...
	rsMailingList.Open();
	// Load the Mailing List Lookup 
	if (!rsMailingList.EOF){ 	
		Response.Write("Grp4Data[107] = new Array();")	
		while (!rsMailingList.EOF) { 
			chrData = rsMailingList("chvName")
			intID   = rsMailingList("insMail_list_id")
	%>
			Grp4Data[107][<%=intID%>] = "<%= chrData %>"
	<%
			rsMailingList.MoveNext 
		}
	} else {
		Response.Write("Mailing List lookup does not exist.")
	}
	...
	function initscr() {
		...
		document.frm04s01.StringSearchOperand.focus();
	}

	function CnstrFltr(output) {	
		// Radio box
		var len = document.frm04s01.SearchType.length;
		var radioGrp = document.frm04s01.SearchType
		var Idparam = -1;
		var stgTemp,j,k; 
		...
		if (output==1) {
			...
			document.frm04s01.action = "m004q01excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter;
			document.frm04s01.target = "_blank";
...
<body onload="initscr()" >
<form name="frm04s01" method="post" onSubmit="CnstrFltr(1);">
<h3>Contact - Quick Search</h3>
<hr>
...
<table cellpadding="1" cellspacing="1">		
    <tr> 
		...
		<td nowrap><DIV ID="oOptrd21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperand" onchange="selectChange(this, frm04s01.LookupValueSearchOperator,detailData);" tabindex="6" style="width: 170px">
			<% 
			while (!rsOprd2.EOF) { 
			%>
...
<table cellpadding="1" cellspacing="1">
	...
	<tr>
		<td nowrap>
			...
			<input type="button" value="Excel" tabindex="12" onClick="CnstrFltr(2);" class="btnstyle">			
