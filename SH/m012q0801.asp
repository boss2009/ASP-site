...
if (String(Request.QueryString("Remove"))=="True") {
	...
	rsRemoveLetter.Open();	
	Response.Redirect("m012q0801.asp?insSchool_id="+Request.QueryString("insSchool_id"));
}

var rsCorrespondence = Server.CreateObject("ADODB.Recordset");
rsCorrespondence.ActiveConnection = MM_cnnASP02_STRING;
rsCorrespondence.Source = "{call dbo.cp_get_pilat_crsp_hstry2("+ Request.QueryString("insSchool_id") + ",0)}";
...
<table cellspacing="1" cellpadding="2" class="Mtable">
    <tr> 
		...
		<th class="headrow" align="left" width="100">Letter Type</th>
		<th class="headrow" align="left" width="150">Sender</th>
		<th class="headrow" align="left" width="150">Method</th>		
		<th class="headrow" align="left" width="100">Date Created</th>
		<th class="headrow">&nbsp;</th>		
    </tr>
<% 
...
%>		
		<td><a href="m012e0803.asp?intLetter_id=<%=(rsCorrespondence.Fields.Item("intLetter_id").Value)%>&insSchool_id=<%=Request.QueryString("insSchool_id")%>"><%=(rsCorrespondence.Fields.Item("chvLetter_Name").Value)%></a></td>
<%
			} else {
%>
...
<%
		...
		case "868":
			//Loan
			if (rsCorrespondence.Fields.Item("intLoan_Req_id").Value > 0) {
%>	
...
<%
		...
		default:
%>
		<td><a href="m012e0801.asp?intLetter_id=<%=(rsCorrespondence.Fields.Item("intLetter_id").Value)%>&insSchool_id=<%=Request.QueryString("insSchool_id")%>&insTemplate_id=<%=rsCorrespondence.Fields.Item("insTemplate_id").Value%>"><%=(rsCorrespondence.Fields.Item("chvLetter_Name").Value)%></a></td>
<%
		break;
	}	
%>
		<td><%=(rsCorrespondence.Fields.Item("chvRx_Type").Value)%>&nbsp;</td>		
		<td><%=(rsCorrespondence.Fields.Item("chvSender").Value)%>&nbsp;</td>
		<td><%=(rsCorrespondence.Fields.Item("chvSend_Method").Value)%>&nbsp;</td>		
		<td><%=FilterDate(rsCorrespondence.Fields.Item("dtsSend_Date").Value)%>&nbsp;</td>		
		<td nowrap><a href="m012q0801.asp?intLetter_id=<%=(rsCorrespondence.Fields.Item("intLetter_id").Value)%>&insSchool_id=<%=Request.QueryString("insSchool_id")%>&Remove=True"><img src="../i/Remove.gif" ALT="Remove <%=(rsCorrespondence.Fields.Item("chvLetter_Name").Value)%> Sent on <%=(rsCorrespondence.Fields.Item("dtsSend_Date").Value)%>"></a></td>				
    </tr>
<%
	Repeat1__index++;
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td><a href="javascript: openWindow('m012a0801.asp?insSchool_id=<%=Request.QueryString("insSchool_id")%>','winAdd');">Add Correspondence</a></td>
	</tr>
</table>
</body>
</html>