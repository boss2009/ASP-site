...
<head>
	...
	function Send(){
		if (!CheckEmail(document.frm0801.Recipient.value)) {
			alert("Invalid recipient email address.");
			document.frm0801.Recipient.focus();
			return ;
		}
		if (Trim(document.frm0801.Recipient.value) == "") {
			alert("Enter recipient email address.");
			document.frm0801.Recipient.focus();
			return ;
		}
		if (!CheckEmail(document.frm0801.Sender.value)) {
			alert("Invalid sender email address.");
			document.frm0801.Sender.focus();
			return ;
		}
		if (Trim(document.frm0801.Sender.value) == "") {
...
As Manager of Sirius  Innovations Inc., I am writing you with regards to <%=Subject_Whole_Name%>'s referral application received by this office.  We are pleased to inform you that <%=Subject_First_Name%> has been accepted for a loan of the following equipment:
<%=ReplaceTags(Loaned_Equipment_List)%>

<%=ReplaceTags(Loan_Conditions)%>
...
Thank you for this referral and we hope that Si2 can be of assistance to you in the future.
