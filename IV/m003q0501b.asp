...
rsBuyoutHistory.Source = "select * from tbl_buyout_equip_sold where intEquip_Set_id = " + Request.QueryString("intEquip_Set_id") + " order by intBuyout_Req_id desc";
...
while ((Repeat1__numRows-- != 0) && (!rsBuyoutHistory.EOF)) {
	...
	var DateProcessed = "";
	var DateDelivered = "";
	var rsBuyoutRequest = Server.CreateObject("ADODB.Recordset");
	rsBuyoutRequest.ActiveConnection = MM_cnnASP02_STRING;
	rsBuyoutRequest.Source = "{call dbo.cp_Buyout_request3("+ rsBuyoutHistory.Fields.Item("intBuyout_Req_id").Value + ",0,0,'',0,'',0,0,0,0,1,'Q',0)}";
	rsBuyoutRequest.CursorType = 0;
	rsBuyoutRequest.CursorLocation = 2;
	rsBuyoutRequest.LockType = 3;
	rsBuyoutRequest.Open();
	
	var intShip_dtl_id = 0;
	if (!rsBuyoutRequest.EOF) {
		if (rsBuyoutRequest.Fields.Item("intShip_dtl_id").Value != null) intShip_dtl_id = rsBuyoutRequest.Fields.Item("intShip_dtl_id").Value;
	} 	
	...
	if (!IsNew) DateDelivered = FilterDate(rsMethod.Fields.Item("dtsDlvy_date").Value);

	switch (String(rsBuyoutRequest.Fields.Item("insEq_user_type").Value)) {
		case "3":
			var rsClient = Server.CreateObject("ADODB.Recordset");
			rsClient.ActiveConnection = MM_cnnASP02_STRING;
			rsClient.Source = "{call dbo.cp_Idv_Adult_Client("+ rsBuyoutRequest.Fields.Item("intEq_user_id").Value + ")}";
			rsClient.CursorType = 0;
			rsClient.CursorLocation = 2;
			rsClient.LockType = 3;
			rsClient.Open();
			if (!rsClient.EOF) SoldTo = Trim(rsClient.Fields.Item("chvFst_Name").Value) + Trim(rsClient.Fields.Item("chvLst_name").Value);
		break;
		case "4":
			var rsInstitution = Server.CreateObject("ADODB.Recordset");
			rsInstitution.ActiveConnection = MM_cnnASP02_STRING;
			rsInstitution.Source = "{call dbo.cp_school2(" + rsBuyoutRequest.Fields.Item("intEq_user_id").Value + ",'',0,0,0,0,0,0,0,'',1,'Q',0)}";
			rsInstitution.CursorType = 0;
			rsInstitution.CursorLocation = 2;
			rsInstitution.LockType = 3;
			rsInstitution.Open();
			if (!rsInstitution.EOF) SoldTo = Trim(rsInstitution.Fields.Item("chvName").Value);
		break;
	}
%>
	<tr>
		...
		<td nowrap align="left"><%=SoldTo%></td>		
		<td nowrap align="left"><%=(rsBuyoutRequest.Fields.Item("chvBO_Status").Value)%></td>		
		<td nowrap align="center"><%=DateProcessed%></td>
		<td nowrap align="center"><%=DateDelivered%></td>		
