...
rsInventory.Source = "{call dbo.cp_Get_EqCls_Inventory_04("+ rsInventory__inspSrtBy.replace(/'/g, "''") + ","+ rsInventory__inspSrtOrd.replace(/'/g, "''") + ",'"+ rsInventory__chvFilter.replace(/'/g, "''") + "',0,0,0)}";
...
<table>
    <tr> 
		<td align="left">Displaying <b><%=(rsInventory_total)%></b> Records.</td>
    </tr>
</table>
<hr>
...
%>
    <tr> 
		<td valign="top" align="left" nowrap><%=Truncate(rsInventory.Fields.Item("chvInventory_Name").Value,40)%></td>
		<td valign="top" align="center" nowrap><%=ZeroPadFormat(rsInventory.Fields.Item("intEquip_Set_id").Value)%></td>
		...
		<td valign="top" align="center" nowrap><%=ZeroPadFormat(rsInventory.Fields.Item("intRequisition_no").Value)%></td>
		<td valign="top" align="left"><%=rsInventory.Fields.Item("chvVendor_Name").Value%></td>
		...
		<td valign="top" align="right" nowrap><%=FormatCurrency(rsInventory.Fields.Item("fltList_Unit_Cost").Value)%></td>
		<td valign="top" align="right" nowrap><%=FormatCurrency(rsInventory.Fields.Item("fltPurchase_Cost").Value)%></td>
		<td valign="top" align="left" nowrap><%=(rsInventory.Fields.Item("chvFunding_Src").Value)%></td>
    </tr>
<%
