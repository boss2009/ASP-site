...
<head>
	...
	function CnstrFltr() {
		var chvOptr = document.frm03p02.ClassNameOperator[document.frm03p02.ClassNameOperator.selectedIndex].value ;
		var chvStg1 = document.frm03p02.ClassName.value;
		var chvStg2 = "";
		var stgFilter = ACfltr_07("1","",chvOptr,chvStg1,"");
		stgPgQuery = "m003p0102.asp?";
		stgPgQuery   += "Search=true&inspSrtBy=1&inspSrtOrd=0&chvFilter=" + stgFilter ;
		document.frm03p02.action = stgPgQuery ;
		document.frm03p02.submit() ; 
	}
	
	function SelectClass(){
		if (document.frm03p02.SearchResult.selectedIndex==-1){
			alert("Select a class.")
			return ;
		}
		if (!top.opener.closed) {				
			top.opener.document.frm03s01.ClassSearchText.value=document.frm03p02.SearchResult.options[document.frm03p02.SearchResult.selectedIndex].text;
			top.opener.document.frm03s01.ClassSearchID.value=document.frm03p02.SearchResult[document.frm03p02.SearchResult.selectedIndex].value;
		}
		top.window.close();
	}
	...
	if (String(Request.QueryString("Search")) == "true") { 
	%>
		document.frm03p02.SearchResult.focus();
	<% 
	} else { 
	%>
		document.frm03p02.ClassNameOperator.focus();
	<% 
	} 
	%>
	}	
...
<body onLoad="init();">
<form name="frm03p02" method="POST" action="">
