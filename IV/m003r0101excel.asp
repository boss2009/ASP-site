...
%>
    <tr> 
		...
		<td><%=ZeroPadFormat(rsInventory.Fields.Item("intEquip_Set_id").Value,8)%></td>
		<td><%=rsInventory.Fields.Item("chvModel_Number").Value%></td>
		<td><%=rsInventory.Fields.Item("chvSerial_Number").Value%></td>
		<td><%=ZeroPadFormat(rsInventory.Fields.Item("intRequisition_no").Value)%></td>
		<td><%=rsInventory.Fields.Item("chvVendor_Name").Value%></td>
		<td><%=(rsInventory.Fields.Item("chvEqp_Status").Value)%></td>
		<td><%=((rsInventory.Fields.Item("chvInstitUsr_Nm").Value=="")?rsInventory.Fields.Item("chvIdvUsr_Nm").Value:rsInventory.Fields.Item("chvInstitUsr_Nm").Value)%></td>
		<td><%=FormatCurrency(rsInventory.Fields.Item("fltList_Unit_Cost").Value)%></td>
		<td><%=FormatCurrency(rsInventory.Fields.Item("fltPurchase_Cost").Value)%></td>
		<td><%=(rsInventory.Fields.Item("chvFunding_Src").Value)%></td>
    </tr>
</table>
</body>
</html>
