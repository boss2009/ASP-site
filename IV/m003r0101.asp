...
rsCol.Source = "{call dbo.cp_ASP_Lkup(724)}";
rsCol.CursorType = 0;
rsCol.CursorLocation = 2;
rsCol.LockType = 3;
rsCol.Open();

// retrieve the singular-select search operands - text
...
// retrieve the singular-select search operands - Combo
...
<html>
<head>
	<title>Inventory Buyout Report</title>
	...
	var rsSchool = Server.CreateObject("ADODB.Recordset");
	rsSchool.ActiveConnection = MM_cnnASP02_STRING;
	rsSchool.Source = "{call dbo.cp_school2(0,'',0,0,0,0,0,1,0,'',2,'Q',0)}";
	rsSchool.CursorType = 0;
	rsSchool.CursorLocation = 2;
	rsSchool.LockType = 3;
	rsSchool.Open();
	// Load the Institution Lookup 
	if (!rsSchool.EOF){ 
		Response.Write("Grp4Data[49] = new Array();")	
		while (!rsSchool.EOF) { 
			chrData = rsSchool("chvSchool_Name")
			intID   = rsSchool("insSchool_id")
	%>
			Grp4Data[49][<%=intID%>] = "<%= chrData %>"
	<%
			rsSchool.MoveNext 
		}
	} else {
	...
	rsPurchase.Open();
	// Load the Purchase Method Lookup 
	if (!rsPurchase.EOF){ 
		Response.Write("Grp4Data[50] = new Array();")	
		while (!rsPurchase.EOF) {
			chrData = rsPurchase("chvPurchase_Method_Desc")
			intID   = rsPurchase("insPurchase_id")
	%>
			Grp4Data[50][<%=intID%>] = "<%= chrData %>"
	<%
			rsPurchase.MoveNext 
		}
	} else {
	...
	rsFunding.MoveFirst;
	// Load the Funding Source Lookup 
	if (!rsFunding.EOF){ 	
	%>
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		...
		if (j==1) {
			selectChange4(frm03s01.LookupValueSearchOperator, frm03s01.LookupValueSearchOptions,Grp4Data );
		}		
		Togo();	  
	}
	...
	function CnstrFltr(type) {			
		...
		if (type==1) {
			document.frm03s01.action = "m003r0101q.asp?inspSrtBy="+intSrtBy+"&inspSrtOrd="+intOrdBy+"&chvFilter=" + stgFilter ;
			document.frm03s01.submit() ; 						
		} else {
			openWindow("m003r0101excel.asp?inspSrtBy="+intSrtBy+"&inspSrtOrd="+intOrdBy+"&chvFilter=" + stgFilter,"");
		}
	}
		...
		switch ( idx ) {
			...
			case "39":
				openWindow("m003p01FS.asp","");
			  	//document.frm03s01.ClassSearchText.value = "362"; 
			break;
			default: 
...
<body onload="initscr()" >
<form name="frm03s01" method="post">
<h5>Inventory Buyout Report</h5>
<hr>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td valign="top"><select name="AndOr" tabindex="17">
