...
if (String(Request("MM_action")) == "update") {
	...
	rsUpdateSchedule.Source = "{call dbo.cp_eqpsrv_ship_schedule("+Request.Form("MM_recordId")+",'"+Request.Form("ScheduledArrivalDate")+"',"+Request.Form("DeliveryOnSchedule")+","+Request.Form("DeliveryStatus")+","+Request.Form("Shipper")+",0,'E',0)}";
...
intShip_Dtl_id = ((String(Request.QueryString("intShip_Dtl_id"))>"0")?Request.QueryString("intShip_Dtl_id"):null);
var rsShippingSchedule = Server.CreateObject("ADODB.Recordset");
rsShippingSchedule.ActiveConnection = MM_cnnASP02_STRING;
rsShippingSchedule.Source = "{call dbo.cp_eqpsrv_ship_schedule("+intShip_Dtl_id+",'',0,0,0,0,'Q',0)}";
...
<head>
	...
	function Init(){
		if (document.frm0403.DeliveryOnSchedule.value=="0") {
			document.frm0403.DeliveryStatus.style.visibility = "visible";
		} else {
			document.frm0403.DeliveryStatus.style.visibility = "hidden";		
		}
		document.frm0403.ScheduledArrivalDate.focus();
	}

	function ChangeDeliveryStatus(){
		if (document.frm0403.DeliveryOnSchedule.value=="0") {
			document.frm0403.DeliveryStatus.style.visibility = "visible";
			document.frm0403.DeliveryStatus.value="1";
		} else {
			document.frm0403.DeliveryStatus.style.visibility = "hidden";		
		}
	}
...
<table cellpadding="1" cellspacing="1">
	<tr>
		...
		<td>
			<input type="text" name="ScheduledArrivalDate" value="<%=FilterDate(rsShippingSchedule.Fields.Item("dtsSch_Arv_date").Value)%>" size="11" maxlength="10" tabindex="1" accesskey="F" onChange="FormatDate(this)">
...
<table cellpadding="1" cellspacing="1">
	<tr> 
		...
		<td><input type="button" value="Close" tabindex="6" onClick="window.location.href='m009e0401.asp?intEquip_srv_id=<%=Request.QueryString("intEquip_srv_id")%>';" class="btnstyle"></td>
    </tr>
</table>
