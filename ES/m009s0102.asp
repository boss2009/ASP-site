...
<html>
<head>
	<title>Equipment Service - Advanced Search</title>
	...
	function selectChange4(control, controlToPopulate,ItemArray) {
		...
		if ((y != 0) && (y!=287) && (y!=288) && (y!=289) && (y!=290)) {
			for ( x = 0 ; x < ItemArray[y].length  ; x++ ) {
				if (ItemArray[y][x]) { 
					myEle = document.createElement("option") ;
					myEle.value = x ;
	...
	function CnstrFltr(output) {	
		...
		var stgFilter = document.frm09s01.QueryString.value;
		if (output==1) {
			document.frm09s01.action = "m009q02.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;		
			document.frm09s01.submit() ; 		
		} else {
			var ExcelSearch = window.open("m009q02excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
	...
	function Savtxt() {
		...
		stgTemp = document.frm09s01.QueryString.value; 
		switch ( Idparam ) {
			//text
			case 0: 
				...
				var chvAO1  = document.frm09s01.AndOr.value ;
			...
			case 1: 
				...
				if ((document.frm09s01.LookupValueSearchOperand.value!="287") && (document.frm09s01.LookupValueSearchOperand.value!="288") && (document.frm09s01.LookupValueSearchOperand.value!="289") && (document.frm09s01.LookupValueSearchOperand.value!="290")) {
			...
			case 3: 
				...
				// Construct filters for multi-item select
				switch (j) {
					// Disability
					case "275" :
						if (stgTemp.length > 0 ) stgTemp += " (" ;
		...
		// update textarea
		document.frm09s01.QueryString.value = stgTemp; 
	}
...
<body onload="initscr()" >
<form name="frm09s01" method="post">
<h3>Equipment Service - Advanced Search</h3>
<hr>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap>Sort by:
			...
			while (!rsCol.EOF) {
			%>
				<option value="<%=(rsCol.Fields.Item("insObjOrder").Value)%>" <%=((rsCol.Fields.Item("insObjOrder").Value == 268)?"SELECTED":"")%>><%=(rsCol.Fields.Item("chvObjName").Value)%></option>
			<%
				rsCol.MoveNext();
			}
			%>
