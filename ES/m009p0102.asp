...
<head>
	...
	function CnstrFltr() {
		var chvOptr = document.frm0902.ClassNameOperator[document.frm0902.ClassNameOperator.selectedIndex].value ;
		var chvStg1 = document.frm0902.ClassName.value;
		var chvStg2 = "";
		var stgFilter = ACfltr_07("1","",chvOptr,chvStg1,"");
		stgPgQuery = "m009p0102.asp?";
		stgPgQuery   += "Search=true&inspSrtBy=1&inspSrtOrd=0&chvFilter=" + stgFilter ;
		document.frm0902.action = stgPgQuery ;
		document.frm0902.submit() ; 
	}
	
	function SelectClass(){
		if (document.frm0902.SearchResult.selectedIndex==-1){
			alert("Select a class.")
			return ;
		}
		if (!top.opener.closed) {				
			top.opener.document.frm09s01.ClassSearchText.value=document.frm0902.SearchResult.options[document.frm0902.SearchResult.selectedIndex].text;
			top.opener.document.frm09s01.ClassSearchID.value=document.frm0902.SearchResult[document.frm0902.SearchResult.selectedIndex].value;
		}
		top.window.close();
	}
	...
	if (String(Request.QueryString("Search")) == "true") { 
	%>
		document.frm0902.SearchResult.focus();
	<% 
	} else { 
	%>
		document.frm0902.ClassNameOperator.focus();
	<% 
	} 
	%>
	}	
...
<body onLoad="init();">
<form name="frm0902" method="POST" action="">
