...
if (Request.QueryString("Remove")=="True") {
	...
	rsDesktop.Open();
	Response.Redirect("m009d01.asp");
}

var rsDesktop = Server.CreateObject("ADODB.Recordset");
rsDesktop.ActiveConnection = MM_cnnASP02_STRING;
rsDesktop.Source = "{call dbo.cp_Idv_Desktop("+ Session("insStaff_id") + ",9,0)}";
rsDesktop.CursorType = 0;
rsDesktop.CursorLocation = 2;
rsDesktop.LockType = 3;
rsDesktop.Open();
...
<html>
<head>
	<title>Equipment Services - Desktop</title>
...
<body>
<h3>Equipment Services - Desktop</h3>
<table cellspacing="1">
	<tr>
...
%>
	<tr> 
		<td nowrap><a href="javascript: openWindow('m009FS3.asp?intEquip_srv_id=<%=(rsDesktop.Fields.Item("intObject_id").Value)%>','wQE09');"><%=((Trim(rsDesktop.Fields.Item("chvInventory_Name").Value)=="")?ZeroPadFormat(rsDesktop.Fields.Item("intObject_id").Value,8):rsDesktop.Fields.Item("chvInventory_Name").Value)%></a></td>
		<td nowrap><%=FilterDate(rsDesktop.Fields.Item("dtmLast_Open").Value)%>&nbsp;</td>
		<td nowrap><a href="m009d01.asp?Remove=True&insDT_id=<%=(rsDesktop.Fields.Item("insDT_id").Value)%>"><img src="../i/remove.gif" ALT="Remove <%=((Trim(rsDesktop.Fields.Item("chvInventory_Name").Value)=="")?ZeroPadFormat(rsDesktop.Fields.Item("intObject_id").Value,8):rsDesktop.Fields.Item("chvInventory_Name").Value)%>"></a></td>				
	</tr>
<%
	Repeat1__index++;
	rsDesktop.MoveNext();
