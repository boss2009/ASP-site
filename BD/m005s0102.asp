...
<html>
<head>
	<title>Bundles - Advanced Search</title>
	...
	function Savtxt() {		
		...
		stgTemp = document.frm05s01.QueryString.value;
		...
		var chvAO1  = document.frm05s01.AndOr.value ;
		if (stgTemp.length > 0 ) {   stgTemp += " (" ;  } 
		stgTemp +=  stgFilter  + ((stgTemp.length > 0) ? ") " : " ") + chvAO1 ;
		document.frm05s01.QueryString.value = stgTemp; 
	}
	...
	function CnstrFltr(output){
		...
		var stgFilter = document.frm05s01.QueryString.value;
		if (output==1) {
			...
			var ExcelSearch = window.open("m005q01excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td valign="top"><select name="AndOr" tabindex="9">
...
<table cellpadding="1" cellspacing="1">
	...
	<tr>
		<td nowrap>
			...
			<input type="button" value="Clear All" onClick="window.location.reload();" tabindex="16" class="btnstyle">
		</td>		
    </tr>
</table>
