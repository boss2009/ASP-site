...
if (Request.QueryString("Remove")=="True") {
	...
	rsDesktop.Open();
	Response.Redirect("m005d01.asp");
}

var rsDesktop = Server.CreateObject("ADODB.Recordset");
rsDesktop.ActiveConnection = MM_cnnASP02_STRING;
rsDesktop.Source = "{call dbo.cp_Idv_Desktop("+ Session("insStaff_id") + ",5,0)}";
rsDesktop.CursorType = 0;
rsDesktop.CursorLocation = 2;
rsDesktop.LockType = 3;
rsDesktop.Open();
...
<head>
	...
	function openWindow(page, name){
		if (page!='nothing') win1=window.open(page, name, "width=480,height=500,scrollbars=1,left=0,top=0,status=1");
		return ;
	}	
	</Script>
</head>
<body>
<h3>Bundle - Desktop</h3>
<table cellspacing="1">
	<tr> 
...
<table cellpadding="2" cellspacing="1" class="Mtable">
    <tr> 
		<th class="headrow" nowrap align="left" width="300">Bundle Name</th>	
...
%>
    <tr> 
		<td nowrap><a href="javascript: openWindow('m005FS3.asp?insBundle_id=<%=(rsDesktop.Fields.Item("intObject_id").Value)%>','wQE01');"><%=(rsDesktop.Fields.Item("chvName").Value)%></a></td>
		<td nowrap><%=FilterDate(rsDesktop.Fields.Item("dtmLast_Open").Value)%>&nbsp;</td>
		<td nowrap><a href="m005d01.asp?Remove=True&insDT_id=<%=(rsDesktop.Fields.Item("insDT_id").Value)%>"><img src="../i/remove.gif" ALT="Remove <%=(rsDesktop.Fields.Item("chvName").Value)%>"></a></td>		
	</tr>
<%
	Repeat1__index++;
	rsDesktop.MoveNext();
