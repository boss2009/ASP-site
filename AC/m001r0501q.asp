...
for (var items=new Enumerator(Request.Form); !items.atEnd(); items.moveNext()) {
  ...
  if (MM_removeList.indexOf(nextItem) == -1) {
    MM_keepURL += "&" + items.item() + "=" + Server.URLencode(Request.Form(items.item()));
  }
}
...
<table cellspacing="1">
    <tr> 
		<td><% if (MM_offset != 0) { %><a href="<%=MM_moveFirst%>?SortByColumn=<%=rsClient__inspSrtBy%>&OrderBy=<%=rsClient__inspSrtOrd%>">First Page</a><% } else { %>First Page<%} // end MM_offset != 0 %>|</td>
		<td><% if (MM_offset != 0) { %><a href="<%=MM_movePrev%>?SortByColumn=<%=rsClient__inspSrtBy%>&OrderBy=<%=rsClient__inspSrtOrd%>">Previous Page</a><% } else { %>Previous Page<% } // end MM_offset != 0 %>|</td>
		<td><% if (!MM_atTotal) { %><a href="<%=MM_moveNext%>?SortByColumn=<%=rsClient__inspSrtBy%>&OrderBy=<%=rsClient__inspSrtOrd%>">Next Page</a><% } else { %>Next Page<% } // end !MM_atTotal %>|</td>
		<td><% if (!MM_atTotal) { %><a href="<%=MM_moveLast%>?SortByColumn=<%=rsClient__inspSrtBy%>&OrderBy=<%=rsClient__inspSrtOrd%>">Last Page</a><% } else { %>Last Page<% } // end !MM_atTotal %></td>
	</tr>
    <tr> 
		<td colspan="4" align="left">Displaying Records <b><%=(rsClient_first)%></b> to <b><%=(rsClient_last)%></b> of <b><%=(rsClient_total)%></b></td>
    </tr>
</table>
<hr>
<table cellpadding="2" cellspacing="1" class="Mtable">
    <tr> 
		...
		<th nowrap class="headrow" align="left">Gender</th>
		<th nowrap class="headrow" align="left">Age</th>
    </tr>
<% 
...
%>
    <tr> 
		...
		<td nowrap><%=(rsClient.Fields.Item("chvDisability").Value)%></td>
		<td nowrap><%=(rsClient.Fields.Item("chvRegion").Value)%></td>
		<td nowrap><%=(rsClient.Fields.Item("chvStatus").Value)%></td>
		<td nowrap><%=FormatSIN(rsClient.Fields.Item("chrSIN_no").Value)%></td>
		<td nowrap><%=(rsClient.Fields.Item("chrGender").Value)%></td>
		<td nowrap><%=(rsClient.Fields.Item("intAge").Value)%></td>
	</tr>
<%
	Repeat1__index++;
	rsClient.MoveNext();
