...
// retrieve the singular-select search operands
var rsOprd1 = Server.CreateObject("ADODB.Recordset");
rsOprd1.ActiveConnection = MM_cnnASP02_STRING;
rsOprd1.Source = "{call dbo.cp_ASP_Lkup2(749,0,'',0,'',0)}";
...
rsOprd2.Source = "{call dbo.cp_ASP_Lkup2(751,0,'',0,'',0)}";
...
rsOprd3.Source = "{call dbo.cp_ASP_Lkup2(750,0,'',0,'',0)}";
...
<html>
<head>
	<title>Client - Grant Eligibility Report</title>
	<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
	<link rel="stylesheet" href="../css/MyStyle.css" type="text/css">
	<script language="JavaScript" src="../js/m001Srh63.js"></script>
	...
	function CnstrFltr(output) {		
		...
		for (var i=0;i <len; i++){
			if (document.frm01s01.SearchType[i].checked == true) Idparam = i;
		}
		...
		switch (Idparam) {
			case 0: 
				...
				if (chvOptr == "0") {
					...
					stgFilter = ACfltr_63(chvOprd,chrNot,chvOptr,chvStg1,chvStg2);
				}
			break;
			case 1:
				stgFilter = ACfltr_63(document.frm01s01.LookupValueSearchOperand.value,"","",document.frm01s01.LookupValueSearchOptions.value,"");							
			break;
			case 2: 
				...
				switch (j) {
					...
					// Financial Source
					case "210":
						stgFilter += " insGrn_Qlf_Src_id IN (" + l + ") " ; 					 
					break;
				}
			default: 
			break;
		...
		switch(output) {
			case 1:
				document.frm01s01.action = "m001r1001q.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;
				document.frm01s01.submit() ; 
			break;
			case 2:
				var SearchExcel = window.open("m001r1001excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
			break;
		}
	}
	...
	function SelOpt() {
		...
		switch ( Idparam ) {
			case 0: 
				...
				oOptr3.style.visibility="hidden";
				oOld3.style.visibility="hidden";
				if ((document.frm01s01.StringSearchOperand.value == "202") || (document.frm01s01.StringSearchOperand.value == "203")){
	...
	rsReferralType.Source = "{call dbo.cp_referring_agent(0,0,0,0,0)}";
	...
	var rsFinancialSource = Server.CreateObject("ADODB.Recordset");
	rsFinancialSource.ActiveConnection = MM_cnnASP02_STRING;
	rsFinancialSource.Source = "{call dbo.cp_grant_qlf_src(0,'',0,'Q',0)}";
	rsFinancialSource.CursorType = 0;
	rsFinancialSource.CursorLocation = 2;
	rsFinancialSource.LockType = 3;
	rsFinancialSource.Open();
	
	if (!rsFinancialSource.EOF){ 
		Response.Write("detailData2[6] = new Array();")	
		while (!rsFinancialSource.EOF) { 
			chrData = Trim(rsFinancialSource("chvGrant_Qlf_Src"))
			intID = rsFinancialSource("insGrant_Qlf_Src")
			Response.Write("detailData2[6]["+intID+"] = '"+chrData+"';");
			rsFinancialSource.MoveNext 
		}
	} else {
		Response.Write("Financial Source lookup does not exist.")
	}
	rsFinancialSource.Close();
...
<h5>Client - Grant Eligibility Report</h5>
<hr>
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		<td nowrap valign="top"><input type="radio" name="SearchType" value="1" checked onClick="SelOpt()" accesskey="F" tabindex="1" class="chkstyle">String Search</td>
		...
		<td nowrap><DIV ID="oStg1" STYLE="visibility:visible">		
			<select name="StringSearchOperator" onChange="Togo();" style="width:130px" tabindex="3"></select>
			<input type="text" name="StringSearchTextOne" tabindex="4">
		</DIV></td>
		<td nowrap><DIV ID="oStg2" STYLE="visibility:hidden">
			<span style="font-size: 7pt">(mm/dd/yyyy)</span>
			<input type="text" name="StringSearchTextTwo" value="<%=CurrentDate()%>" tabindex="5" DISABLED>
            <span style="font-size: 7pt">(mm/dd/yyyy)</span>
		</DIV></td>
    </tr>
</table>
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		<td nowrap valign="top"><input type="radio" name="SearchType" value="2" onClick="SelOpt()" tabindex="6" class="chkstyle">Lookup Value Search</td>
		<td nowrap valign="top"><DIV ID="oOptrd2" STYLE="visibility:visible"> 
			<select name="LookupValueSearchOperand" tabindex="7">
			<% 
			while (!rsOprd2.EOF) {
			%>
		...
		<td nowrap valign="top" colspan="2"><DIV ID="oOptr2" STYLE="visibility:hidden">
			is: <select name="LookupValueSearchOptions" width="150" align="top" tabindex="8">
			<% 
			while (!rsCaseManager.EOF) {
			%>
				<option value="<%=(rsCaseManager.Fields.Item("insId").Value)%>"><%=(rsCaseManager.Fields.Item("chvName").Value)%></option>
			<%
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		<td nowrap valign="top"><input type="radio" name="SearchType" value="3" onClick="SelOpt()" tabindex="9" class="chkstyle">Multi Value Search</td>
		<td nowrap valign="top"><DIV ID="oOptrd3" STYLE="visibility:visible"> 
			<select name="MultiValueSearchOperand" OnChange="initializeList('frm01s01.MultiValueSearchOperand','frm01s01.MultiValueSearchText')" tabindex="10">
			<% 
			while (!rsOprd3.EOF) {
			%>
			...
			<DIV ID="oOld3" STYLE="visibility:hidden"><input type="checkbox" name="IncludeHistoricValues" value="1" onClick="Selckb()" tabindex="11" class="chkstyle">Include Historic Values</DIV>
		</DIV></td>
		<td nowrap valign="top" colspan="2"><DIV ID="oOptr3" STYLE="visibility:hidden">
			contains: <select name="MultiValueSearchText" size="8" width="150" multiple align="top" tabindex="12"></select>
		</DIV></td>
    </tr>
</table>
<br>
<table cellpadding="1" cellspacing="1">
	...
	<tr>
		<td nowrap>
			...
			<input type="button" value="Clear All" onClick="window.location.reload();" tabindex="17" class="btnstyle">
		</td>		
    </tr>
</table>
