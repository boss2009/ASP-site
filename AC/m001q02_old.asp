...
<%
var rsClient__chvFilter = "e.dtsRefral_date >= '01/01/1900'";
...
rsClient.Source = "{call dbo.cp_Adult_Client2E("+ Request.QueryString("inspSrtBy") + ","+ Request.QueryString("inspSrtOrd") + ",'"+ rsClient__chvFilter.replace(/'/g, "''") + "')}";
...
<table cellspacing="1">
  ...
  <div class="BrowsePanel" style="width: 100%; height: 340px"> 
    <table cellpadding="2" cellspacing="1">
      <tr> 
        <th nowrap class="headrow" align="left" width="180">Name</th>
        <th nowrap class="headrow" align="left">&nbsp;</th>
        <th nowrap class="headrow" align="left" width="80">SIN</th>
        <th nowrap class="headrow" align="left" width="130">Status</th>
        ...
        <th nowrap class="headrow" align="left">Referral Type</th>
        <th nowrap class="headrow" align="left">Referring Agent</th>
<!--
...
function FormatPhone(phone){
	if ((Trim(phone) == "") || (phone == null)) return "";
	phone = LeaveDigits(phone);
...
%>
      <tr> 
        <td nowrap><a href="javascript: openWindow('m001FS3.asp?intAdult_id=<%=(rsClient.Fields.Item("intAdult_Id").Value)%>','wQE01');"><%=(rsClient.Fields.Item("chvLst_Name").Value)%>, <%=(rsClient.Fields.Item("chvFst_Name").Value)%></a></td>
        <td nowrap><a href="javascript: openWindow('m001q01j.asp?intAdult_id=<%=rsClient.Fields.Item("intAdult_Id").Value%>','Summary');"><img src="../i/summary.gif" ALT="<%=(rsClient.Fields.Item("chvLst_Name").Value)%>, <%=(rsClient.Fields.Item("chvFst_Name").Value)%> summary information"></a></td>
        <td nowrap><%=FormatSIN(rsClient.Fields.Item("chrSIN_no").Value)%>&nbsp;</td>
        <td nowrap><%=(rsClient.Fields.Item("chvStatus").Value)%>&nbsp;</td>
<!-- + Oct.28.2005
        <td nowrap><%=FormatPhoneNumber("Hm",rsClient.Fields.Item("chvPhone1_Arcd").Value,rsClient.Fields.Item("chvPhone1_Num").Value,rsClient.Fields.Item("chvPhone1_Ext").Value,"","","","","","","","")%>&nbsp;</td>
-->
        ...
        <td nowrap align="center"> 
          <%if (rsClient.Fields.Item("bitRef_Type").Value=="1") { %>
          <a href="javascript: openWindow('m001pop1.asp?intAdult_id=<%=rsClient.Fields.Item("intadult_id").Value%>','wP01');"><img src="../i/summary.gif" alt="Show Referral Type For <%=rsClient.Fields.Item("chvLst_Name").Value%>, <%=rsClient.Fields.Item("chvFst_Name").Value%>"></a> 
          <%}%>
        </td>
        <td nowrap align="center"> 
          <%if (rsClient.Fields.Item("bitRef_Agent").Value=="1") { %>
          <a href="javascript: openWindow('m001pop2.asp?intAdult_id=<%=rsClient.Fields.Item("intadult_id").Value%>','wP02');"><img src="../i/summary.gif" alt="Show Referring Agent For <%=rsClient.Fields.Item("chvLst_Name").Value%>, <%=rsClient.Fields.Item("chvFst_Name").Value%>"></a> 
          <%}%>
        </td>
<!--
        <td nowrap><%=FilterDate(rsClient.Fields.Item("dtsRefral_date").Value)%>&nbsp;</td>
        <td nowrap><%=FilterDate(rsClient.Fields.Item("dtsRe_refral_date").Value)%>&nbsp;</td>
-->
<% 
	if ( Session("MM_UserAuthorization") >= 6 ) { 
%>
