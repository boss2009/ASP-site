...
if (String(Request.QueryString("Search"))=="true") {
	var rsEquipmentClass__inspSrtBy = ((String(Request.QueryString("inspSrtBy")) != "undefined")?Request.QueryString("inspSrtBy"):"1");
	var rsEquipmentClass__inspSrtOrd = ((String(Request.QueryString("inspSrtOrd")) != "undefined")?Request.QueryString("inspSrtOrd"):"0");
	var rsEquipmentClass__chvFilter = ((String(Request.QueryString("chvFilter")) != "undefined")?Request.QueryString("chvFilter"):"");
	var rsEquipmentClass = Server.CreateObject("ADODB.Recordset");
	rsEquipmentClass.ActiveConnection = MM_cnnASP02_STRING;
	rsEquipmentClass.Source = "{call dbo.cp_EC_Eqp_Class("+ rsEquipmentClass__inspSrtBy + ","+ rsEquipmentClass__inspSrtOrd + ",'"+ String(rsEquipmentClass__chvFilter).replace(/'/g, "''") + "')}";
...
<head>
	...
	function SelectClass(){
		...
		if (!top.opener.closed) {
			top.opener.document.frm01s01.ClassSearchText.value=document.frm0101.SearchResult.options[document.frm0101.SearchResult.selectedIndex].text;
			top.opener.document.frm01s01.ClassSearchID.value=document.frm0101.SearchResult[document.frm0101.SearchResult.selectedIndex].value;
		}			
		top.window.close();
	}
