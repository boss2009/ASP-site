...
<head>
	...
	function CnstrFltr(output) {		
		...
		if (output==1) {
			document.frm01s01.action = "m001r0101q.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;
			document.frm01s01.submit();
		} else {
			var SearchExcel = window.open("m001r0101excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
	...
	function Savtxt() {	
		...
		switch (Idparam) {
			...
			case 2: 
				l = "";
				var count = 0;
				var optList = document.frm01s01.MultiValueSearchOptions;
				...
				if (optList.multiple) {
					for(var ii = 0; ii < m; ii++) {
						if (document.frm01s01.MultiValueSearchOptions[ii].selected) {		
							if (l.length > 0 ) l += "," ;
							l += document.frm01s01.MultiValueSearchOptions[ii].value ;		
						} 				  
					} 						  
				} else {
					l = document.frm01s01.MultiValueSearchOptions[document.frm01s01.MultiValueSearchOptions.selectedIndex].value
				} 
				if (stgTemp.length > 0 ) stgTemp += " (" ;
				blnFlg = true;				
...
<h5>Funding Source, Service, Date Range Report</h5>
<hr>
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap valign="center"><DIV ID="oStg3" STYLE="visibility:hidden">
			Contains: 
			<select name="MultiValueSearchOptions" size="8" width="150" align="top" multiple tabindex="7">
			<%			
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap>Sort by:
			<select name="SortByColumn" tabindex="11">
			<% 
			while (!rsCol.EOF) {
			%>
        	...
        	<select name="OrderBy" tabindex="12">
	...
	<tr>
		<td nowrap>
	        <input type="button" value="Search" onClick="CnstrFltr(1);" tabindex="13" class="btnstyle">
			<input type="button" value="Excel" onClick="CnstrFltr(2);" tabindex="14" disabled class="btnstyle">
			<input type="button" value="Clear All" onClick="window.location.reload();" tabindex="15" class="btnstyle">
		</td>		
    </tr>
</table>
</form>
