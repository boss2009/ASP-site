...
rsNotesServices.Source = "{call dbo.cp_get_adultclient_srv_note("+ Request.QueryString("intAdult_id") + ",0)}";
...
for (var i = 0; i < count; i++) {
	...
	NotesServicesArray[i][2] = rsNotesServices.Fields.item("intStd_note_id").Value;	
	NotesServicesArray[i][3] = rsNotesServices.Fields.item("intService_Req_id").Value;	
	NotesServicesArray[i][4] = rsNotesServices.Fields.item("chrYear").Value;	
	NotesServicesArray[i][5] = rsNotesServices.Fields.item("chrCycle").Value;	
	NotesServicesArray[i][6] = rsNotesServices.Fields.item("chvSrv_Prvder").Value;	
...
<head>
	...
	function openWindow(page, name){
		if (page!='nothing') win1=window.open(page, name, "width=550,height=500,scrollbars=1,left=0,top=0,status=1");
		return ;
	}		
	</Script>	
</head>
<body>
<h5>Services and Notes</h5>
<table cellspacing="1">
	<tr>
		<td width="160">Displaying All of <%=count%> Records.</td>
		<td width="130"><a href="javascript: openWindow('m001a1201.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>','wA1202');">Add Service Request</a></td>
		<td width="90"><a href="javascript: openWindow('m001a1202.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>','wA1202');">Add Notes</a></td>
<%
if (Request.QueryString("ShowFS")=="1") {
%>		
		<td nowrap><a href="m001q1201.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>&ShowFS=0">Hide Funding Source</a></td>
<%
} else {
%>
		<td nowrap><a href="m001q1201.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>&ShowFS=1">Show Funding Source</a></td>
<%
}
%>
	</tr>
...
%>
    <tr>
		<td valign="top"><a href="m001e1202.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>&intSrv_Note_id=<%=NotesServicesArray[i][1]%>&intStd_note_id=<%=NotesServicesArray[i][2]%>"><%=FilterDate(NotesServicesArray[i][0])%></a>&nbsp;</td>
...
%>
    <tr>
		<td valign="top"><a href="m001e1201.asp?intAdult_id=<%=Request.QueryString("intAdult_id")%>&intSrv_Note_id=<%=NotesServicesArray[i][1]%>&intService_Req_id=<%=NotesServicesArray[i][3]%>"><%=FilterDate(NotesServicesArray[i][0])%></a>&nbsp;</td>
