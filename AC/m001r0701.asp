...
<html>
<head>
	<title>Clients with specfic equipment purchased</title>
	...
	function CnstrFltr(output) {
		...
		if (output==1) {
			document.frm01s01.action = "m001r0701q.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;
			document.frm01s01.submit(); 
		} else {
			var ExcelSearch = window.open("m001r0701excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
	...
	if (!rsRegion.EOF){ 
		...
		while (!rsRegion.EOF){ 
			Response.Write("detailData2[1]["+rsRegion("insRegion_num")+"] = '"+rsRegion("chvname")+"';");
			rsRegion.MoveNext 
		}
	} else {
...
<h5>Clients with specfic equipment purchased</h5>
<hr>
...
<table cellpadding="1" cellspacing="1">	
    <tr> 
		...
		<% 
		while (!rsOprd3.EOF) {
			if (rsOprd3.Fields.Item("intRecID").Value=="20"){
		%>
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap valign="top"><DIV ID="oOptrd4" STYLE="visibility:visible"> 
			...
			<DIV ID="oOld4" STYLE="visibility:hidden"><input type="checkbox" name="IncludeHistoricValues" value="1" onClick="Selckb()" tabindex="16" class="chkstyle">Include Historic Values</DIV>
		</DIV></td>
