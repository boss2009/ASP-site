...
<head>
	...
	function JumpRecord(){
		if (document.frm0701.JumpToRecord.value=="") {
			alert("Enter Record Number.");
		}
		if (!IsID(document.frm0701.JumpToRecord.value)) {
			alert("Invalid Record Number.");
		} else {
			window.location.href="..<%Response.Write(Request.ServerVariables("URL") + "?" + MM_keepMove + MM_moveParam + "=")%>"+String(document.frm0701.JumpToRecord.value-1);
		}
	}				
	</Script>
</head>
