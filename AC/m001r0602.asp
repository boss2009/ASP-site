...
rsOprd4.Source = "{call dbo.cp_ASP_Lkup(716)}";
...
<html>
<head>
	<title>Clients with specfic equipment on loan</title>
	<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
	<link rel="stylesheet" href="../css/MyStyle.css" type="text/css">
	<script language="JavaScript" src="../js/m001Srh03.js"></script>
	...
	function CnstrFltr(output) {
		...
		if (output==1) {
			document.frm01s01.action = "m001r0602q.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;
			document.frm01s01.submit(); 
		} else {
			var ExcelSearch = window.open("m001r0602excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
	...
	function initscr() {
		...
		oOptrd4.style.visibility="hidden";
		oOptr4.style.visibility="hidden";
		initializeList('frm01s01.MultiValueSearchOperand','frm01s01.MultiValueSearchText');	
		oOld4.style.visibility="hidden";
	...
	function SelOpt() {
		...
		switch ( Idparam ) {
			...
			case 3: 
				...
				oStg21.style.visibility="hidden";
				oOptrd31.style.visibility="hidden";												
				oOptrd4.style.visibility="visible";
				oOptr4.style.visibility="visible";
				oOld4.style.visibility="visible";
			break;
		}       	
	}
	...
	function Savtxt() {	
		...
		switch (Idparam) {
			...
			case 1: 
				...
				stgFilter = ACfltr_03(chvOprd,"","",chvStg1,"");
				if (stgTemp.length > 0 ) stgTemp += " (" ;
				stgTemp +=  stgFilter  + ((stgTemp.length > 0) ? ") " : " ") + chvAO1 ;				
			break;			
			case 2: 
				if (document.frm01s01.ClassSearchOperand.length>=1) {
					j = document.frm01s01.ClassSearchOperand[document.frm01s01.ClassSearchOperand.selectedIndex].value ;
				} else {
					alert("Select Class Search Operand.");
					document.frm01s01.ClassSearchOperand.focus();
					break;
				}
				if (document.frm01s01.ClassSearchID.value < 1) {
					alert("Select an equipment class.");
					break;
				}
				var chvOprd = j;
				var chvAO1  = document.frm01s01.AndOr.value ;				
				var chvStg1 = document.frm01s01.ClassSearchID.value;
				stgFilter = ACfltr_03(chvOprd,"","",chvStg1,"");
				if (stgTemp.length > 0 ) stgTemp += " (" ;
				stgTemp +=  stgFilter  + ((stgTemp.length > 0) ? ") " : " ") + chvAO1 ;												
			break;						
		...
		document.frm01s01.StringSearchTextTwo.value="";		
		if (blnFlg) {
			var chvAO1  = document.frm01s01.AndOr.value ;
			stgTemp +=  stgFilter  + ((stgTemp.length > 0) ? ") " : " ") + chvAO1 + " ";
		}
...
<h5>Clients with specfic equipment on loan</h5>
<hr>
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap valign="top"><DIV ID="oStg12" STYLE="visibility:hidden">
...
<table cellpadding="1" cellspacing="1">	
    <tr> 
		...
		<% 
		while (!rsOprd3.EOF) {
			if (rsOprd3.Fields.Item("intRecID").Value=="19"){		
		%>
				<option value="<%=(rsOprd3.Fields.Item("intRecID").Value)%>"><%=(rsOprd3.Fields.Item("chvObjName").Value)%></option>
		<%
			}
			rsOprd3.MoveNext();
		}
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap valign="top">
			<DIV ID="oOptrd4" STYLE="visibility:visible"> 
				<select name="MultiValueSearchOperand" OnChange="initializeList('frm01s01.MultiValueSearchOperand','frm01s01.MultiValueSearchText')" tabindex="15">
				<% 
				while (!rsOprd4.EOF) {
				%>
				...
				<DIV ID="oOld4" STYLE="visibility:hidden">
					<input type="checkbox" name="IncludeHistoricValues" value="1" onClick="Selckb()" tabindex="16" class="chkstyle">Include Historic Values
				</DIV>
			</DIV>
		</td>
		<td nowrap valign="top" colspan="2"><DIV ID="oOptr4" STYLE="visibility:hidden"> 
			contains: <select name="MultiValueSearchText" size="8" width="150" multiple align="top" tabindex="17"></select>
		</DIV></td>
    </tr>
</table>
<table cellpadding="1" cellspacing="1">
	<tr>
		<td valign="top"><select name="AndOr" tabindex="18">
...
<table cellpadding="1" cellspacing="1">
    <tr> 
		<td valign="top" nowrap><input type="button" name="AddToQueryString" value="Add" tabindex="19" onClick="Savtxt()" class="btnstyle"></td>
		<td valign="top" nowrap><textarea name="QueryString" cols="80" rows="6" tabindex="20" accesskey="L"></textarea></td>
    </tr>
</table>
<br>
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap>Sort by:
			<select name="SortByColumn" tabindex="21">
			<% 
			while (!rsCol.EOF) {
			%>
        	...
        	<select name="OrderBy" tabindex="22">
	...
	<tr>
		<td nowrap>
	        <input type="button" value="Search" onClick="CnstrFltr(1);" tabindex="23" class="btnstyle">
			<input type="button" value="Excel" onClick="CnstrFltr(2);" tabindex="24" class="btnstyle">
			<input type="button" value="Clear All" onClick="window.location.reload();" tabindex="25" class="btnstyle">
		</td>		
    </tr>
</table>
