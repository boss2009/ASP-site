...
rsClient.Source = "{call dbo.cp_Adult_Client3B("+ Request.QueryString("inspSrtBy") + ","+ Request.QueryString("inspSrtOrd") + ",'"+ rsClient__chvFilter.replace(/'/g, "''") + "')}";
...
<head>
	...
	function JumpRecord(){
		if (document.frm0602.JumpToRecord.value=="") {
			alert("Enter Record Number.");
		}
		if (!IsID(document.frm0602.JumpToRecord.value)) {
			alert("Invalid Record Number.");
		} else {
			window.location.href="..<%Response.Write(Request.ServerVariables("URL") + "?" + MM_keepMove + MM_moveParam + "=")%>"+String(document.frm0602.JumpToRecord.value-1);
		}
	}				
	</Script>
</head>
  ...
  <div class="BrowsePanel" style="height: 340px; width: 100%"> 
    <table cellpadding="2" cellspacing="1" class="Mtable">
      <tr> 
        ...
        <th nowrap class="headrow" align="left">SIN</th>
...
function FormatPhone(phone){
	...
	return "(" + phone.substring(0,3) + ") " + phone.substring(4,7) + "-" + phone.substring(7,11);
}
...
%>
      <tr> 
        ...
        <td nowrap align="center"><a href="javascript: openWindow('m001q01j.asp?intAdult_id=<%=rsClient.Fields.Item("intAdult_Id").Value%>','Summary');"><img src="../i/summary.gif" ALT="<%=(rsClient.Fields.Item("chvLst_Name").Value)%>, <%=(rsClient.Fields.Item("chvFst_Name").Value)%> summary information"></a></td>
        <td nowrap align="left"><%=FormatSIN(rsClient.Fields.Item("chrSIN_no").Value)%>&nbsp;</td>
        <td nowrap align="left"><%=(rsClient.Fields.Item("chvStatus").Value)%>&nbsp;</td>
        <td nowrap align="left"><%=FormatPhone(rsClient.Fields.Item("chvPhone_no").Value)%>&nbsp;</td>
        <td nowrap align="left"><%=(rsClient.Fields.Item("chvCaseManager").Value)%>&nbsp;</td>
