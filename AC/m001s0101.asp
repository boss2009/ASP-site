...
// retrieve the text search operands
...
<html>
<head>
	<title>Client - Quick Search</title>
	...
	function CnstrFltr(output) {		
		...
		switch (Idparam) {
			...
			case 2: 
				...
				switch (j) {
					...
					break;
					case "32": 
						alert("Service Code option not available ..."); 
					break;
				}
			break;
			default: 
		...
		switch(output) {
			...
			break;
			case 2:
				document.frm01s01.action = "m001q02excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter;
				document.frm01s01.target = "_blank";
				document.frm01s01.submit();
			break;
			case 3:			
				document.frm01s01.action = "m001q02txt.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;
				document.frm01s01.target = "_blank";
				document.frm01s01.submit();				
			break;
		}
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		...
		document.frm01s01.StringSearchTextOne.size = 20;
		document.frm01s01.StringSearchTextTwo.size = 20;						
		var y = control.value;		
		switch(y){
			case "13":
				document.frm01s01.StringSearchTextOne.style.visibility = "hidden";			
			...
			case "18":
				document.frm01s01.StringSearchTextTwo.disabled = false;		
				document.frm01s01.StringSearchTextOne.style.visibility = "visible";
				document.frm01s01.StringSearchTextOne.size = 12;
				document.frm01s01.StringSearchTextTwo.size = 12;
				oStg2.style.visibility="visible";			
			break;
			case "21":
				document.frm01s01.StringSearchTextOne.style.visibility = "hidden";			
	...
	if (!rsDisability.EOF){ 
		...
		while (!rsDisability.EOF){ 
			Response.Write("detailData2[2]["+rsDisability("insDisability_id")+"] = '"+rsDisability("chvname")+"';");
	...
	if (!rsStatus.EOF){ 
		...
		while (!rsStatus.EOF){ 
			Response.Write("detailData2[3]["+rsStatus("insStdnt_status_id")+"] = '"+rsStatus("chvName")+"';");
			rsStatus.MoveNext 
		}
	} else {
	...
	if (!rsCaseManager.EOF){ 
		...
		while (!rsCaseManager.EOF){ 
			Response.Write("detailData2[13]["+rsCaseManager("insId")+"] = '"+rsCaseManager("chvName")+"';");
	...
	if (!rsReferringAgent.EOF){ 
		...
		while (!rsReferringAgent.EOF){ 
			Response.Write("detailData2[4]["+rsReferringAgent("insrefer_agent_id")+"] = '"+rsReferringAgent("chvname")+"';");
	...
	if (!rsFundingSource.EOF){ 
		...
		while (!rsFundingSource.EOF) { 
			Response.Write("detailData2[5]["+rsFundingSource("insFunding_source_id")+"] = '"+rsFundingSource("chvfunding_source_name")+"';");
	...
	if (!rsServiceType.EOF){ 
		...
		while (!rsServiceType.EOF) { 
			Response.Write("detailData2[6]["+rsServiceType("insService_type_id")+"] = '"+rsServiceType("chvname")+"';");
	...
	if (!rsRegion2.EOF){ 	
		...
		while (!rsRegion2.EOF){ 
			Response.Write("detailData2[7]["+rsRegion2("insRegion_num")+"] = '"+ rsRegion2("chvname")+"';");
			rsRegion2.MoveNext 
		}
	} else {
		Response.Write("Region lookup does not exist.")
	...
	if (!rsDisability2.EOF){ 	
		...
		while (!rsDisability2.EOF){ 
			Response.Write("detailData2[8]["+rsDisability2("insDisability_id")+"] = '"+rsDisability2("chvname")+"';");
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap><DIV ID="oStg1" STYLE="visibility:visible">		
			<select name="StringSearchOperator" onChange="Togo();" style="width:80px" tabindex="4"></select>
...
<table cellpadding="1" cellspacing="1" border="0">
    <tr> 
		...
		<td nowrap valign="top"><DIV ID="oOptrd3" STYLE="visibility:visible"> 
			<select name="MultiValueSearchOperand" OnChange="initializeList('frm01s01.MultiValueSearchOperand','frm01s01.MultiValueSearchText')" style="width: 150px" tabindex="11">
			<% 
			while (!rsOprd3.EOF) {
			%>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap>
			Sort by:
			<select name="SortByColumn" tabindex="14">
			<% 
			while (!rsCol.EOF) {
			%>
        	...
        	<select name="OrderBy" tabindex="15">
	...
	<tr>
		<td nowrap>
	        <input type="submit" value="Search" onClick="CnstrFltr(1);" tabindex="16" class="btnstyle">
			<input type="button" value="Excel" onClick="CnstrFltr(2);" tabindex="17" class="btnstyle">
			<input type="button" value="Text" onClick="CnstrFltr(3);" tabindex="18" class="btnstyle">
			<input type="button" value="Clear All" onClick="window.location.reload();" tabindex="19" class="btnstyle">
		</td>		
    </tr>
</table>
