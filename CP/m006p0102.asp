...
<head>
	...
	function CnstrFltr() {
		var chvOprd = "1" ; 
		var chrNot  = "";
		var chvOptr = document.frm06p02.ClassNameOperator[document.frm06p02.ClassNameOperator.selectedIndex].value ;
		var chvStg1 = document.frm06p02.ClassName.value;
		var chvStg2 = "";
		var stgFilter = ACfltr_07("1","",chvOptr,chvStg1,"");
		stgPgQuery = "m006p0102.asp?";
		stgPgQuery   += "Search=true&inspSrtBy=1&inspSrtOrd=0&chvFilter=" + stgFilter ;
		document.frm06p02.action = stgPgQuery ;
		document.frm06p02.submit() ; 
	}
	
	function SelectClass(id, name){
		if (document.frm06p02.SearchResult.selectedIndex==-1){
			alert("Select a class.")
			return ;
		}	
		if (!top.opener.closed) {	
			top.opener.document.frm06s01.ClassName.value=document.frm06p02.SearchResult.options[document.frm06p02.SearchResult.selectedIndex].text;
			top.opener.document.frm06s01.ClassID.value=document.frm06p02.SearchResult[document.frm06p02.SearchResult.selectedIndex].value;
		}
		top.window.close();
	}
	...
	if (String(Request.QueryString("Search")) == "true") { 
	%>
		document.frm06p02.SearchResult.focus();
	<% 
	} else { 
	%>
		document.frm06p02.ClassNameOperator.focus();
	<% 
	} 
	%>
	}	
...
<body onLoad="init();">
<form name="frm06p02" method="POST" action="">
