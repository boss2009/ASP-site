...
<head>
	...
	function Init() {
		...
		selectChange(frm0103.Operand, frm0103.Operator,detailData);
	<%
	if (String(Request.QueryString("Search")) == "true") { 		
	%>
		document.frm0103.SearchResult.focus();
	<%
	} else {
	%>
		document.frm0103.Operand.focus();
	<%
	}
	%>
	}
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		document.frm0103.StringSearchTextBoxOne.value = "";
		document.frm0103.StringSearchTextBoxTwo.value = "";
		var myEle ;
		for (var q=controlToPopulate.options.length;q>=0;q--) controlToPopulate.options[q]=null;
		myEle = document.createElement("option") ;
		var y = control.value;
		if ((y == "18") || (y == "22")){
			document.frm0103.StringSearchTextBoxTwo.disabled = false;
			oStg2.style.visibility = "visible";
		} else {
			document.frm0103.StringSearchTextBoxTwo.disabled = true;
			oStg2.style.visibility = "hidden";
		}
		if (y == "17") {
	...
	function Togo() {
		var j = document.frm0103.Operand[document.frm0103.Operand.selectedIndex].value ;
		var l = document.frm0103.Operator[document.frm0103.Operator.selectedIndex].value	
		document.frm0103.MM_curOprd.value = j ;
		document.frm0103.MM_curOptr.value = l ;
		document.frm0103.MM_flag.value = true ;
	}

	function CnstrFltr() {
		var stgPgQuery = "";		
		var chvOprd = document.frm0103.Operand[document.frm0103.Operand.selectedIndex].value ; 
		var chvOptr = document.frm0103.Operator[document.frm0103.Operator.selectedIndex].value ;
		var chvStg1 = document.frm0103.StringSearchTextBoxOne.value;
		var chvStg2 = document.frm0103.StringSearchTextBoxTwo.value;

		if (((chvOprd=="18") || (chvOprd=="22")) && (chvOptr!="0")) {
			if (chvStg1 == "") {
				alert("Enter Start Date");
				document.frm0103.StringSearchTextBoxOne.focus();
				return ;
			}
			if (chvStg2 == "") {
				alert("Enter End Date");
				document.frm0103.StringSearchTextBoxTwo.focus();
				return ;
			}
		...
		if (chvOptr == "0") {
			...
			var stgFilter = ACfltr_01(chvOprd,"",chvOptr,chvStg1,chvStg2);
			stgPgQuery += "m022a0103.asp?Search=true&inspSrtBy=1&inspSrtOrd=0&chvFilter=" + stgFilter ;
		}
		document.frm0103.action = stgPgQuery;
		document.frm0103.submit();
	}
	
	function SelectClient(id,name){
		window.location.href='m022a0102.asp?IsNew=No&intAdult_id='+id;
	}
	</script>
</head>
<body onload="Init();">
<form name="frm0103" method="post" action="">
<h5>Search Criteria</h5>
<table cellpadding="1" cellspacing="1">
	<tr> 
		<td nowrap>
			<select name="Operand" onchange="selectChange(this, frm0103.Operator,detailData);" tabindex="1" accesskey="F">
			<% 
			while (!rsOprd.EOF) {
			%>
...
<table cellpadding="1" cellspacing="1">
	...
	<tr>
		<td>
			<input type="button" value="Select Client" tabindex="8" onClick="SelectClient(document.frm0103.SearchResult[document.frm0103.SearchResult.selectedIndex].value,document.frm0103.SearchResult.options[document.frm0103.SearchResult.selectedIndex].text);" <%=((count==0)?"DISABLED":"")%> class="btnstyle">
