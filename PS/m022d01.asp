...
if (Request.QueryString("Remove")=="True") {
	...
	rsDesktop.Open();
	Response.Redirect("m022d01.asp");
}

var rsDesktop = Server.CreateObject("ADODB.Recordset");
rsDesktop.ActiveConnection = MM_cnnASP02_STRING;
rsDesktop.Source = "{call dbo.cp_Idv_Desktop("+ Session("insStaff_id") + ",22,0)}";
rsDesktop.CursorType = 0;
rsDesktop.CursorLocation = 2;
rsDesktop.LockType = 3;
rsDesktop.Open();
...
<html>
<head>
	<title>Temp Student - Desktop</title>
...
<body>
<h3>Temp Student - Desktop</h3>
<table cellspacing="1">
	<tr> 
		<td><% if (MM_offset != 0) { %>	
        <div align="center"><a href="<%=MM_moveFirst%>"><img src="../i/First.gif" width="18" height="13"></a>		          
          <% } else { %> 
            <img src="../i/FirstPage.gif" width="30" height="30">		        </div>
		      <%	} // end MM_offset != 0 %> |</td>
		<td><% if (MM_offset != 0) { %>	<a href="<%=MM_movePrev%>"><img src="../i/Previous.gif" width="14" height="13"></a> 
		      <% } else { %>	<img src="../i/PreviousPage.gif" width="30" height="30"> 
		      <% } // end MM_offset != 0 %>	|</td>
		<td><% if (!MM_atTotal) { %> <a href="<%=MM_moveNext%>"><img src="../i/Next.gif" width="14" height="13"></a> 
		      <% } else { %><img src="../i/NextPage.gif" width="30" height="30"> 
		      <% } // end !MM_atTotal %> |</td>
		<td><% if (!MM_atTotal) { %> <a href="<%=MM_moveLast%>"><img src="../i/Last.gif" width="18" height="13"></a> 
		      <% } else { %> <img src="../i/LastPage.gif" width="30" height="30"> 
		      <% } // end !MM_atTotal %></td>
	</tr>
    <tr> 
...
<table cellpadding="2" cellspacing="1" class="Mtable">
    <tr> 
		<th class="headrow" nowrap align="left" width="250">Temp Student</th>	
...
%>
    <tr> 
		<td nowrap><a href="javascript: openWindow('m022FS3.asp?intPStdnt_id=<%=(rsDesktop.Fields.Item("intObject_id").Value)%>','wQE01');"><%=(rsDesktop.Fields.Item("chvStdent_Name").Value)%></a></td>
		<td nowrap><%=FilterDate(rsDesktop.Fields.Item("dtmLast_Open").Value)%>&nbsp;</td>
		<td nowrap><a href="m022d01.asp?Remove=True&insDT_id=<%=(rsDesktop.Fields.Item("insDT_id").Value)%>"><img src="../i/remove.gif" ALT="Remove <%=(rsDesktop.Fields.Item("chvStdent_Name").Value)%>"></a></td>				
	</tr>
<%
	Repeat1__index++;
	rsDesktop.MoveNext();
