...
rsOprd.Source = "{call dbo.cp_ASP_Lkup2(754,0,'',0,'0',0)}";
rsOprd.CursorType = 0;
...
rsOprd2.Source = "{call dbo.cp_ASP_Lkup2(755,0,'',0,'0',0)}";
...
rsOprd3.Source = "{call dbo.cp_ASP_Lkup2(756,0,'',0,'0',0)}";
...
<head>
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		...
		if (j==1) {
			selectChange4(frm08s01.LookupValueSearchOperator, frm08s01.LookupValueSearchOptions,Grp4Data );
		}	
		Togo();	 
	}
	//-------
	...
	function CnstrFltr(output) {	
		...
		if (output==1) {
			...
			document.frm08s01.action = "m008q02excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter;
			document.frm08s01.target = "_blank";
			document.frm08s01.submit();
		}
	}
...
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td><DIV ID="oOptrd21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperand" onchange="selectChange(this, frm08s01.LookupValueSearchOperator,detailData);" tabindex="9">
			<% 
			while (!rsOprd2.EOF) { 
			%>
		...
		<td><DIV ID="oOptr21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperator" onchange="selectChange4(this, frm08s01.LookupValueSearchOptions,Grp4Data );" tabindex="10"></select>
		</DIV></td>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		...
		<td nowrap valign="top"><DIV ID="oOptrd31" STYLE="visibility:visible"> 
			<select name="MultiSelectOperand" OnChange="initializeList(document.frm08s01.MultiSelectOperand,document.frm08s01.MultiSelectOptions)" tabindex="13">
			<% 
			while (!rsOprd3.EOF) {
			%>
