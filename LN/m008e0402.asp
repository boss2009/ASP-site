...
if (String(Request.Form("MM_Update"))=="true") {
	...
	rsNotes.Source = "{call dbo.cp_eqp_request_notes(0,"+Request.QueryString("int_Note_id")+",'"+Notes+"',"+ Session("insStaff_id")+",'"+1+"',0,'E',0)}";
	rsNotes.CursorType = 0;
	rsNotes.CursorLocation = 2;
	rsNotes.LockType = 3;
	rsNotes.Open();
	rsNotes.Close();
	Response.Redirect("UpdateSuccessful.asp?page=m008q0402.asp&intLoan_req_id="+Request.QueryString("intLoan_req_id"));
}

var rsNotes = Server.CreateObject("ADODB.Recordset");
rsNotes.ActiveConnection = MM_cnnASP02_STRING;
rsNotes.Source = "{call dbo.cp_eqp_request_notes(0,"+Request.QueryString("int_Note_id")+",'',0,'',1,'Q',0)}";
rsNotes.CursorType = 0;
rsNotes.CursorLocation = 2;
rsNotes.LockType = 3;
rsNotes.Open();
