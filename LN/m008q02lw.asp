...
<hr>
	<a id=first class="blue" href="m008q01lw.asp?intLoan_req_id=<%=Request.QueryString("intLoan_req_id")%>&intEqpReq_Id=<%=Request.QueryString("intEqpReq_Id")%>">.. Back To Root</a><br>
<%
while (!rsSubAbstractClass.EOF){ 
%>
	<a href="m008q03lw.asp?ClassID=<%=rsSubAbstractClass.Fields.Item("insEquip_Class_id").Value%>&ParentID=<%=rsSubAbstractClass.Fields.Item("insSuper_Class_id").Value%>&intLoan_req_id=<%=Request.QueryString("intLoan_req_id")%>&intEqpReq_Id=<%=Request.QueryString("intEqpReq_Id")%>"><img src="../i/collapse.gif" align="absmiddle" ALT="Expand SubAbstract Class <%=rsSubAbstractClass.Fields.Item("chvName").Value%>"></a><%=rsSubAbstractClass.Fields.Item("chvName").Value%><br>		
<%
