...
rsNotes.Source = "{call dbo.cp_eqp_request_notes("+ Request.QueryString("intLoan_req_id") + ",0,'',0,'',0,'Q',0)}";
...
%>
    <tr> 
		<td valign="top"><a href="m008e0402.asp?int_Note_id=<%=(rsNotes.Fields.Item("int_Note_id").Value)%>&intLoan_req_id=<%=Request.QueryString("intLoan_req_id")%>"><%=(rsNotes.Fields.Item("chvType_of_Note").Value)%></a></td>		
...
switch (ChkNotes.Parameters.Item("@insRtnFlag").Value) {
	case 0:
%>
<a href="javascript: openWindow('m008a0402.asp?intLoan_req_id=<%=Request.QueryString("intLoan_req_id")%>','wA0402');">Add Requested Notes</a>
<%
	break ;
	case 1:
%>
