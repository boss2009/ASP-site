...
rsOprd.Source = "{call dbo.cp_ASP_Lkup2(754,0,'',0,'3',0)}";
rsOprd.CursorType = 0;
...
rsOprd2.Source = "{call dbo.cp_ASP_Lkup2(755,0,'',1,'3',0)}";
...
<html>
<head>
	<title>Loan - Referral Report</title>
	...
	%>
			Grp4Data[186][<%=rsLoanType("intloan_type_id")%>] = "<%= rsLoanType("chvname") %>"
	<%
			rsLoanType.MoveNext 
		}
	} else {
	...
	%>
			Grp4Data[215][<%=rsCaseManager("insId")%>] = "<%= rsCaseManager("chvName") %>"
	<%
	...
	%>
			Grp4Data[185][<%=rsReferralType("insrefer_agent_id")%>] = "<%= rsReferralType("chvname") %>"
	<%
	...
	%>
			Grp4Data[183][<%=rsLoanStatus("intloan_status_id")%>] = "<%= rsLoanStatus("chvname") %>"
	<%
	...
	%>
			Grp4Data[184][<%=rsUserType("insEq_user_type")%>] = "<%= rsUserType("chvEq_user_type") %>"
	<%
			rsUserType.MoveNext 
		}
	} else {
	...
	Grp4Data[219] = new Array();
	Grp4Data[219][0] = "Unable to Arrange";
	Grp4Data[219][1] = "Declined";
	Grp4Data[219][2] = "Completed";	

	Grp4Data[220] = new Array();
	Grp4Data[220][0] = "Unable to Arrange";
	Grp4Data[220][1] = "Declined";
	Grp4Data[220][2] = "Completed";	
	...
	function Savtxt(output) {	
		...
		switch ( Idparam ) {
			case 0: 
				...
				if (((chvOprd=="174") || (chvOprd=="177") || (chvOprd=="178") || (chvOprd=="217") || (chvOprd=="218")) && (chvOptr!="0")) {
					if (chvStg1 == "") {
	...
	function CnstrFltr(output){
		...
		if (output==1) {
			document.frm08s01.action = "m008r0101q.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter ;		
			document.frm08s01.submit() ; 		
		} else {
			var ExcelSearch = window.open("m008r0101excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter);
		}
	}
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap><select name="AndOr" tabindex="17">
...
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td nowrap><textarea name="QueryString" cols="80" rows="6" tabindex="19" accesskey="L"></textarea></td>
    </tr>
</table>
<br>
<table cellpadding="1" cellspacing="1">
