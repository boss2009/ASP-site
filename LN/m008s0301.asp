...
rsOprd.Source = "{call dbo.cp_ASP_Lkup2(754,0,'',0,'2',0)}";
rsOprd.CursorType = 0;
...
rsOprd4.Source = "{call dbo.cp_ASP_Lkup2(756,0,'',1,'',0)}";
...
<head>
	...
	function initscr() {
		...
		var j = 0;
		var len = document.frm08s01.SearchType.length;		
		for (var i=0;i <len; i++) if (document.frm08s01.SearchType[i].checked) j = i;
		switch (j) {
			case 0:
	...
	function Savtxt(output) {	
		...
		switch ( Idparam ) {
			...
			case 0: 
				...
				if (((chvOprd=="174") || (chvOprd=="177") || (chvOprd=="178")) && (chvOptr!="0")) {
					if (chvStg1 == "") {
...
<table cellpadding="1" cellspacing="1">
	<tr>
		...
		<td nowrap valign="top"><DIV ID="oOptr41" STYLE="visibility:hidden"> 
			contains: <select name="MultiSelectOptions" size="8" style="width: 150px" multiple align="top" tabindex="16"></select>
		</DIV></td>		
	</tr>
</table>
<table cellpadding="1" cellspacing="1">
