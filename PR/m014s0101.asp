<!--------------------------------------------------------------------------
* File Name: m014s0101.asp
* Title: Purchase Requisition - Quick Search
* Main SP: 
* Description: Purchase Requisition Quick Search.
* Author: T.H
--------------------------------------------------------------------------->
<%@language="JAVASCRIPT"%>
...
rsOprd2.Source = "{call dbo.cp_ASP_Lkup(731)}";
...
<html>
<head>
	<title>Purchase Requisition - Quick Search</title>
	...
	function selectChange(control, controlToPopulate,ItemArray) {
		...
		switch(y){
			case "78" :
				document.frm14s01.StringSearchTextboxTwo.disabled = false;
			...
			break;
			default :
				document.frm14s01.StringSearchTextboxTwo.disabled = true;
		...
		document.frm14s01.StringSearchTextboxOne.value = "";
		...
		if (j==1) {
			selectChange4(frm14s01.LookupValueSearchOperator, frm14s01.LookupValueSearchOptions,Grp4Data);
		}		
		Togo();	  
	}
	...
	function CnstrFltr(output) {	
		...
		switch ( Idparam ) {
			case 0: 
				...
				var chvStg1 = document.frm14s01.StringSearchTextboxOne.value;
				var chvStg2 = document.frm14s01.StringSearchTextboxTwo.value;
		...
		if (chvOptr == "0") {
			...
			if (output==1) {
				...
				document.frm14s01.action = "m014q01excel.asp?inspSrtBy="+inspSrtBy+"&inspSrtOrd="+inspSrtOrd+"&chvFilter=" + stgFilter;
				document.frm14s01.target = "_blank";
	...
	function SelectOperator() {
		if (document.frm14s01.StringSearchOperator.value=="21") {
			document.frm14s01.StringSearchTextboxTwo.disabled = false;
			document.frm14s01.StringSearchTextboxTwo.value = "";			
			oImg11.style.visibility="visible";		
		} else {
			document.frm14s01.StringSearchTextboxTwo.disabled = true;
			document.frm14s01.StringSearchTextboxTwo.value = "";			
...
<h3>Purchase Requisition - Quick Search</h3>
<hr>
<table cellpadding="1" cellspacing="1">
    <tr> 
		...
		<td nowrap><DIV ID="oImg11" STYLE="visibility:hidden">
			<input type="text" name="StringSearchTextboxTwo" value="<%=CurrentDate()%>" tabindex="5" size="10">
		</DIV></td>
    </tr>
</table>
<table cellpadding="1" cellspacing="1">
    <tr> 
		<td nowrap width="160"><input type="radio" name="SearchType" value="2" onClick="SelOpt()" tabindex="6" class="chkstyle">Lookup Value Search</td>
		<td nowrap><DIV ID="oOptrd21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperand" onchange="selectChange(this, frm14s01.LookupValueSearchOperator,detailData);" tabindex="7">
			<% 
			while (!rsOprd2.EOF) { 
			%>
		...
		<td nowrap><DIV ID="oOptr21" STYLE="visibility:visible">
			<select name="LookupValueSearchOperator" onchange="selectChange4(this, frm14s01.LookupValueSearchOptions,Grp4Data );" tabindex="8"></select>
		</DIV></td>
...
<table cellpadding="1" cellspacing="1">
	<tr>
		<td nowrap>Sort by:
			<select name="SortByColumn" tabindex="10">
			<% 
			while (!rsCol.EOF) {
			%>
			...
			Order 
        	<select name="OrderBy" tabindex="11">
	...
	<tr>
		<td nowrap>
	        <input type="submit" value="Search" onClick="CnstrFltr(1);" tabindex="12" class="btnstyle">
			<input type="button" value="Excel" onClick="CnstrFltr(2);" tabindex="13" class="btnstyle">			
			<input type="reset" value="Clear All" tabindex="14" onClick="window.location.reload();" class="btnstyle">
		</td>		
    </tr>
</table>
