<!--------------------------------------------------------------------------
* File Name: m014a0401.asp
* Title: New Notes
* Main SP: cp_Purchase_Requisition_Note
* Description: The page combines adding requested notes and received notes
* into one.
* Author: T.H
--------------------------------------------------------------------------->
<%@language="JAVASCRIPT"%>
...
if (String(Request.Form("MM_Insert"))=="true") {
	...
	rsNotes.Source = "{call dbo.cp_Purchase_Requisition_Note("+Request.QueryString("insPurchase_Req_id")+",'"+Notes+"',0,"+ Session("insStaff_id")+",'"+rsNotes__Notes_type+"',0,'A',0)}";
...
<html>
<head>
	<title>New <%=((Request.QueryString("NotesType")=="Requested")?"Requested":"Received")%> Notes</title>
...
<h5>New <%=((String(Request.QueryString("NotesType"))=="Requested")?"Requested":"Received")%> Notes</h5>
<hr>
...
<input type="hidden" name="NotesType" value="<%=((String(Request.QueryString("NotesType"))=="Requested")?"Requested":"Received")%>">
</form>
</body>
</html>