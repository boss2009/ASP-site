<!--------------------------------------------------------------------------
* File Name: m014d01.asp
* Title: Purchase Requisition - Desktop
* Main SP: cp_idv_desktop
* Description: This page displays the content of user's purchase requisition
* desktop.
* Author: T.H
--------------------------------------------------------------------------->
<%@language="JAVASCRIPT"%>
...
if (Request.QueryString("Remove")=="True") {
	...
	rsDesktop.Open();
	Response.Redirect("m014d01.asp");
}

var rsDesktop = Server.CreateObject("ADODB.Recordset");
rsDesktop.ActiveConnection = MM_cnnASP02_STRING;
rsDesktop.Source = "{call dbo.cp_Idv_Desktop("+ Session("insStaff_id") + ",14,0)}";
rsDesktop.CursorType = 0;
rsDesktop.CursorLocation = 2;
rsDesktop.LockType = 3;
rsDesktop.Open();
...
<html>
<head>
	<title>Purchase Requisition - Desktop</title>
...
<body>
<h3>Purchase Requisition - Desktop</h3>
<table cellspacing="1">
	<tr>
...
<table cellpadding="2" cellspacing="1" class="Mtable">
    <tr> 
		<th class="headrow" nowrap align="left">PR Number</th>
		<th class="headrow" nowrap align="left">Date Opened</th>
...
%>
	<tr> 
		<td nowrap><a href="javascript: openWindow('m014FS3.asp?insPurchase_Req_id=<%=(rsDesktop.Fields.Item("intObject_id").Value)%>','wQE01');"><%=ZeroPadFormat(rsDesktop.Fields.Item("intObject_id").Value,8)%></a></td>
		<td nowrap><%=FilterDate(rsDesktop.Fields.Item("dtmLast_Open").Value)%>&nbsp;</td>
		<td nowrap><a href="m014d01.asp?Remove=True&insDT_id=<%=(rsDesktop.Fields.Item("insDT_id").Value)%>"><img src="../i/remove.gif" ALT="Remove <%=(rsDesktop.Fields.Item("intObject_id").Value)%>"></a></td>
	</tr>
<%
	Repeat1__index++;
	rsDesktop.MoveNext();
